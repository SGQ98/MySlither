...
final class MySlitherWebSocketClient extends WebSocketClient {
    ...
    private void processLeaderboard(int[] data) {
        ...
        if (ownRank > 10) {
            double totalLength = model.getSnakeLength(model.snake.body.size(), model.snake.getFam());
            view.setHighscoreData(rank, model.snake.name + " (Rank: " + ownRank + ")", (int) totalLength, true);
        }
    }
    ...
    private void processAddFood(int[] data, boolean allowMultipleEntities, boolean fastSpawn) {
        ...
        for (int i = 8; i < data.length; i += 6) {
            ...
            model.addFood(color, x, z, radius, fastSpawn); // TODO: now always...
        }
    }
